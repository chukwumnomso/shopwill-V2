...
export default function HeroText() {
  return (
    <div className="flex flex-col gap-8 absolute h-full w-full px-2 md:px-4 lg:px-7 justify-center items-start  text-white bg-linear-to-b from-black/10 to-black/80 ">
      ...
      <div>
        <p className="animate-slide-2 text-lg font-[jost]  max-w-[80%] sm:max-w-[60%] md:max-w-[50%] md:text-xl tracking-wider">
          Where street meets style, you are the trend. Shop the hottest picks
          now & make a statement.
        </p>

        <Button className="size-12 text-white border-none cursor-pointer hover:bg-white hover:text-black transition-colors duration-500  bg-black font-[jost] text-xl font-bold animate-slide-3 border-2 border-white sm:w-[60%] md:w-[50%] w-[80%] mt-4">
          Shop Now
        </Button>
      </div>
    </div>
